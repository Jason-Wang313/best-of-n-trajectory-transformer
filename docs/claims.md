...
- In a synthetic Trajectory Transformer-style token model, increasing N can increase selected reward-token return while lowering support likelihood and increasing dynamics inconsistency.
- The exact finite-N selection law for score-selected i.i.d. candidates matches Monte Carlo in the included test.
- The Support-Calibrated Plan Sieve can improve realized utility or block unsafe high-N candidate sets in the controlled experiments.
