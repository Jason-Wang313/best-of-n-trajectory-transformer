...
def write_final_audit(summary: dict, pdf_repo_path: Path, pdf_download_path: Path) -> None:
    ...
    lines = [
        ...
        "",
        f"   In the `{empirical_variant}` control, raw Best-of-N changed predicted return by {main.get('raw_predicted_gain_1_to_64', 0.0):.3f} from N=1 to N=64 while realized return changed by {main.get('raw_realized_change_1_to_64', 0.0):.3f}. This is synthetic evidence for reward-token extremization, not a benchmark claim.",
        "",
        "5. **What is the strongest repair result?**",
        "",
        f"   The strongest repair result is in `{strongest_variant}`: at N=64, the Support-Calibrated Plan Sieve improved realized return over raw selection by {strongest.get('sieve_minus_raw_realized_at_64', 0.0):.3f} or blocked unsafe candidate sets when support diagnostics crossed calibration thresholds.",
        "",
...
def run_suite(base: ExperimentConfig, variants: tuple[str, ...] | None = None) -> dict:
    ...
    if summary:
        write_final_audit(
            ...
            pdf_download_path=Path.home() / "Downloads" / "iclr_submission_trajectory_transformer.pdf",
        )
    return payload
